...
export function calculateEnergyBoltDamage(caster, time) {
    ...
    const rawValues = results.map(r => r.raw);
    const flooredValues = results.map(r => r.floored);

    const avgRaw = rawValues.reduce((sum, val) => sum + val, 0) / rawValues.length;
    const min = Math.min(...flooredValues);
    const max = Math.max(...flooredValues);
